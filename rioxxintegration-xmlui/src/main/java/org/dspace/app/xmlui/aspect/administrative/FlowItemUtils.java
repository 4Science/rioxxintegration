...
public class FlowItemUtils
{
    ...
    public static FlowResult processAddFunding(Context context, UUID itemID, Request request) throws SQLException, AuthorizeException {
    	Item item = itemService.find(context, itemID);
        ...
        if (project != null) {
        	itemService.addMetadata(context, item,"rioxxterms", "identifier", "project", null, project.getValue(), project.getId(), Choices.CF_ACCEPTED);
            String id = project.getFunderAuthorityValue().getId();
            if (StringUtils.isNotBlank(funderAuthority)) {
                id = funderAuthority;
            }
            itemService.addMetadata(context, item,"rioxxterms", "funder", null, null, project.getFunderAuthorityValue().getValue(),
    ...
    public static FlowResult processRemoveFunding(Context context, UUID itemID, Request request) throws SQLException, AuthorizeException {
    	Item item = itemService.find(context, itemID);
        ProjectService projectService = new DSpace().getServiceManager().getServiceByName("ProjectService", ProjectService.class);
        ...
        FlowResult result = new FlowResult();
        removeProjectAndFunderFromItem(context, item, projectID, funderID);
    ...
    private static void removeProjectAndFunderFromItem(Context context, Item item, String projectID, String funderID) throws SQLException {
        ...
        for(MetadataValue metadatum : dcValues){
            if(!StringUtils.equals(metadatum.getAuthority(), projectID)){
                itemService.addMetadata(context, item, "rioxxterms", "identifier", "project", metadatum.getLanguage(), metadatum.getValue(), metadatum.getAuthority(), metadatum.getConfidence());
            }
        }

        List<MetadataValue> funderValues = itemService.getMetadata(item, "rioxxterms", "funder",null, Item.ANY);
        itemService.clearMetadata(context, item, "rioxxterms", "funder", null, Item.ANY);
        for(MetadataValue metadatum : funderValues){
            if(!StringUtils.equals(metadatum.getAuthority(),funderID)){
                itemService.addMetadata(context, item, "rioxxterms", "funder", null , metadatum.getLanguage(), metadatum.getValue(), metadatum.getAuthority(), metadatum.getConfidence());
            }
        }
    }
